...
function clearError(popup) { // функция очистки ошибок валидации в случае, если попап был закрыт без сохранения
  const inputs = [...popup.querySelectorAll(`.${formConfig.inputErrorClass}`)];
  const errors = [...popup.querySelectorAll(`.${formConfig.errorClass}`)];
  inputs.forEach((input) => {
    input.classList.remove(formConfig.inputErrorClass); // убрать стиль ошибки для инпута
  });
